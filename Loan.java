public class Loan {
    ...
    public Loan(double annualInterestRate, int numberOfYears, double loanAmount) throws IllegalArgumentException {
        if (annualInterestRate <= 0)
            throw new IllegalArgumentException("Недопустимая годовая процентная ставка");

        if (numberOfYears <= 0)
            throw new IllegalArgumentException("Недопустимое количество лет");

        if (loanAmount <= 0)
            throw new IllegalArgumentException("Недопустимая сумма кредита");
